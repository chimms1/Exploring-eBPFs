...
echo "=> Compiling infostealmagnumopus.bpf.c"
clang -g -O2 -target bpf -D__TARGET_ARCH_x86 -c infostealmagnumopus.bpf.c -o infostealmagnumopus.bpf.o  -D__BPF_TRACING__
...
docker build -t 3-infosteal .

echo "=> Container built, running now"
docker run -it --privileged 3-infosteal